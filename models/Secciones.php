...
class Secciones extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('titulo, url, created, modified', 'required'),
			array('titulo', 'length', 'max'=>45),
			array('url', 'length', 'max'=>255),
			array('descripcion', 'length', 'allowEmpty'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('idsecciones, titulo, url, descripcion, created, modified', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('idsecciones',$this->idsecciones);
