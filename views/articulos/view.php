...
<div class="texto_izq_nov">
  ...
  <div class="compartir_nov">
    <div class="compartir_btn" style="border: none;"><div class="fb-like" data-href="<?php echo $_SERVER["SERVER_NAME"].Yii::app()->request->url; ?>" data-width="74" data-layout="button_count" data-action="like" data-show-faces="false" data-share="false"></div></div>
...
<div class="linea_gris_nov_inf"></div>
    ...
    <h2 class="titulo_gal_secciones">Artículos</h2>
	<div class="caja_gris_nov">
		<div class="caja_titulos_nov1y2">
		<?php 
			$i=0;
                    for ($i; $i < count($articulos) && $i < 6; $i++) {
                        if ($model->idarticulo == $articulos[$i]->idarticulo) {
                            echo CHtml::link('<h2 class="texto_titulo_nov" style="color:#bbb;">' . $articulos[$i]->titulo . '</h2>', '/articulos/view/' . $articulos[$i]->idarticulo . "#titulo");
                        } else {
                            echo CHtml::link('<h2 class="texto_titulo_nov">' . $articulos[$i]->titulo . '</h2>', '/articulos/view/' . $articulos[$i]->idarticulo . "#titulo");
                        }
                    }
		?>
	    </div>
	    <div class="caja_titulos_nov1y2">
		<?php 
                    for ($i; $i < count($articulos) && $i < 12; $i++) {
                        if ($model->idarticulo == $articulos[$i]->idarticulo) {
                            echo CHtml::link('<h2 class="texto_titulo_nov" style="color:#bbb;">' . $articulos[$i]->titulo . '</h2>', '/articulos/view/' . $articulos[$i]->idarticulo . "#titulo");
                        } else {
                            echo CHtml::link('<h2 class="texto_titulo_nov">' . $articulos[$i]->titulo . '</h2>', '/articulos/view/' . $articulos[$i]->idarticulo . "#titulo");
