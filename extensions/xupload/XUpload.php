...
class XUpload extends CJuiInputWidget {
    ...
    public function run() {
        ...
        $this->options['url_fotos'] = $this->url_fotos;
		...
		Yii::app() -> clientScript -> registerScript(__CLASS__ . '#' . $this -> htmlOptions['id'],
			"
		        ...
		        $('#{$this->htmlOptions['id']}').addClass('fileupload-processing');
		        $.ajax({
		            url: '$this->url_fotos',
		            dataType: 'json',
		            context: $('#{$this->htmlOptions['id']}')[0]
		        }).always(function () {
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            $messages = CJavaScript::encode(array(
                'fileupload' => array(
                    'errors' => array(
                        "maxFileSize" => $this->t('Archivo es muy grande'),
                        "minFileSize" => $this->t('Archivo es muy pequeño'),
                        "acceptFileTypes" => $this->t('Tipo de archivo no permitido'),
                        "maxNumberOfFiles" => $this->t('Excede numero maximo de archivos'),
                        "uploadedBytes" => $this->t('Cantidas de Bytes excede el maximo permitido'),
                        "emptyResult" => $this->t('Archivo vacío'),
                    ),
                    'error' => $this->t('Error'),
                    'start' => $this->t('Iniciar'),
                    'cancel' => $this->t('Cancelar'),
                    'destroy' => $this->t('Eliminar'),
                ),
            ));
