...
class NewsletterController extends RController {
    ...
    public function actionEnviar($id) {
        ...
        $pathNews = realpath(Yii::app()->getBasePath() . "/../images/newsletter/" . $id . "/") . "/";
        $publicNews = Yii::app()->getBaseUrl() . "/images/newsletter/" . $id . "/";
        $imgNews = array();
        if (is_dir($pathNews)) {
            if ($gestor = opendir($pathNews)) {
                while (false !== ($entrada = readdir($gestor))) {
                    if (is_file($pathNews . $entrada) && strpos($entrada, '.') > 0) {
                        $imgNews[] = rawurlencode($entrada);
                    }
                }
            }
        }
        ...
        $correos = Recibenovedades::model()->findAll('activo=1 group by email');

        $enviados = 0;
        $por_enviar = count($correos);
        
        while ($enviados < $por_enviar) {
            $message2 = new YiiMailMessage;
            $message2->view = 'novedades';
            $message2->setBody(array('model' => $model, 'imagen' => $imgNews), 'text/html');
            $message2->subject = $model->titulo;
            $message2->addTo('0800cocinas@gmail.com');
            $message2->from = '0800cocinas@gmail.com';
            for ($i=0; ($i<99 && $enviados<$por_enviar); $i++){
                $message2->addBCC($correos[$enviados]->email);
                $enviados++;
            }
    ...
    public function actionPrueba($id) {
        ...
        $message2->setBody(array('model' => $model, 'imagen' => $imgNews), 'text/html');
        $message2->subject = $model->titulo;
        $message2->addTo("fabricacocinaslacuisine@gmail.com");
        $message2->addTo("0800cocinas@gmail.com");
        $message2->addTo("cgpm2007@gmail.com");
        $message2->addTo("burli2000@gmail.com");
        $message2->from = '0800cocinas@gmail.com';
        Yii::app()->mail->send($message2);
        unset($message2);
        $model->activo = 1;
