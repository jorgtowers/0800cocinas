...
class NosotrosController extends Controller {
    ...
    public function actionView($id) {
        ...
        $publicidad = Publicidadnosotros::model()->findAll('activo=1 order by RAND()');
        $arr_pub = array();

        $pathPUB = realpath(Yii::app()->getBasePath() . "/../images/publicidadnosotros/") . "/";
        $publicPathPUB = Yii::app()->getBaseUrl() . "/images/publicidadnosotros/";
        ...
        $this->render('view', array(
            'model' => $this->loadModel($id), 'nosotros' => $nosotros, 'galeria' => $galeria, 'rawDataTips' => $rawDataTips, 'publicidad' => $arr_pub, 'recibenovedades' => $recibenovedades
        ));
    }
    ...
    public function actionFirstGallery($id) {
        ...
        $principal = '';
        $images = '';
        $path = realpath(Yii::app()->getBasePath() . "/../images/nosotros/" . $id . "/") . "/";
        $publicPath = Yii::app()->getBaseUrl() . "/images/nosotros/" . $id . "/";
                ...
                for ($i = 0; $i < $cant; $i++) {
                    if ($i === 0 && $cant === 1) {
                        $principal .= '<div class="cuadro_transparente_nov">
								  <div class="imagen1_ejem_nov"><img src="' . $arr_img[$i] . '" border="0" id="imgnosotros" width="565"></div>
									</div>';
    ...
    public function loadModel($id) {
        $model = Nosotros::model()->findByPk($id);
        if ($model === null)
