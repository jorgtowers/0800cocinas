...
class EventosController extends Controller
{
	...
	public function actionView($id)
	{
		...
		$eventos = Eventos::model()->findAll('activo=1 order by created DESC limit 12');
		
		$publicidad = Publicidadeventos::model()->findAll('activo=1 order by RAND()');
		$arr_pub = array();
		
		$pathPUB = realpath( Yii::app( )->getBasePath( )."/../images/publicidadeventos/" )."/";
    	$publicPathPUB = Yii::app( )->getBaseUrl( )."/images/publicidadeventos/";
    	...
    	$this->render('view',array(
			'model'=>$this->loadModel($id),'eventos'=>$eventos,'galeria'=>$galeria,'publicidad'=>$arr_pub,'recibenovedades'=>$recibenovedades
		));
	...
	public function actionFirstGallery($id)
	{
		...
		$principal = '';
		$images = '';
		$path = realpath( Yii::app( )->getBasePath( )."/../images/eventos/".$id."/" )."/";
	    $publicPath = Yii::app( )->getBaseUrl( )."/images/eventos/".$id."/";
						...
						for ($i=0;$i<$cant;$i++){
				    		if ($i === 0 && $cant === 1){
				    			$principal .= '<div class="cuadro_transparente_nov">
								  <div class="imagen1_ejem_nov"><img src="'.$arr_img[$i].'" border="0" id="imgeventos" width="565"></div>
									</div>';
